...
template
class tep::Unique_Ptr<int>;

template
class tep::Unique_Ptr<B>;

template
class tep::Unique_Ptr<A>;
...
template<typename T>
void Unique_Ptr<T>::reset(const T& a_ptr) noexcept
{
...
template<typename T>
void Unique_Ptr<T>::swap(const Unique_Ptr& a_ptr) noexcept
{
    std::iter_swap(m_ptr, a_ptr.m_ptr);
}
